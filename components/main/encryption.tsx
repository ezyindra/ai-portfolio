...
export const Encryption = () => {
  ...
  const [isUnlocked, setIsUnlocked] = useState(false); // ✅ added unlock state
  ...
  return (
    <section className="relative min-h-[100svh] w-full flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-20 flex flex-col items-center justify-center">
        <EncryptionModal
          videoRef={videoRef}
          onUnlock={() => setIsUnlocked(true)}   // ✅ unlock callback
          trigger={
      ...
      {/* 🤖 AI Assistant appears after unlock */}
      {isUnlocked && (
        <motion.div
          variants={slideInFromTop}
          initial="hidden"
          animate="show"
          className="absolute bottom-24 w-full flex justify-center z-20 px-6"
        >
          <AIChat />
        </motion.div>
      )}
